...
bool Config::ParseLogInfo(const Json::Value &root)
{
  ...
  if (!rtlObj.isNull())
  {
    ...
    {
      log_info_->rotate_type = kRotateNone;
    }
  }
  return true;
}
...
bool tmms::base::Config::LoadFile(const std::string &file, Json::Value &cfg)
{
  ...
  Json::CharReaderBuilder reader;
  std::string errs;
  if (!Json::parseFromStream(reader, ifs, &cfg, &errs))
  {
    return false;
  }
  return true;
